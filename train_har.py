...
def main(args: argparse.Namespace) -> None:
    """
    Summary: Main function to train model on HAR benchmark
    ...
    if not args.inter and args.source == "realworld":
        args.n_classes = 8
        transform = [Cast(8)]
    print("\n=========Preparing Data=========")
    ...
    if args.inter:
        ...
        label_list, _ = tf.unique(
            [tf.math.argmax(elem["label"]) for elem in train_dataset],
        )
        valid_datasets = {
            d: valid_datasets[d].filter(Filter_label(label_list))
            for d in valid_datasets
        }
        valid_datasets = {
            d: valid_datasets[d].apply(
                tf.data.experimental.assert_cardinality(
                    len([i for i, elem in enumerate(valid_datasets[d])]),
                ),
            )
            for d in valid_datasets
        }
        ...
        train_dataset = get_dataset(
            ...
            train=True,
            transform=transform,
            source_localisation=args.source_localisation,
        )
        valid_datasets = get_dataset(
            args.source,
            path=data_dir,
            inter=False,
            train=False,
    ...
    valid_datasets = {
        d: valid_datasets[d].shuffle(2048).batch(args.batch_size)
        for d in valid_datasets
    }
...
if __name__ == "__main__":
    ...
    from src.datasets.transforms import Cast, Filter_label
