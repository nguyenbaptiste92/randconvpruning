...
def main(args):
    ...
    if args.multi_aug:
        train_transform = [Cast(args.n_classes),Rescale(255),
            Reshape(image_size),
            GreyToColor(),
    ...
    test_transform = [Cast(args.n_classes),Rescale(255),
            Reshape(image_size),
            GreyToColor(),
            Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
        ]
    ...
    train_dataset = get_dataset(args.source, path=data_dir, train=True, transform=train_transform)
    valid_datasets = {domain: get_dataset(domain, path=data_dir, train=False, transform=test_transform) for domain in domains}

    train_dataset = train_dataset.shuffle(4096,reshuffle_each_iteration=True).batch(args.batch_size)
    valid_datasets = {d: valid_datasets[d].shuffle(4096,reshuffle_each_iteration=True).batch(512) for d in valid_datasets}
    ...
    trainer.train(net, train_dataset, valid_datasets, data_mean=(0.5, 0.5, 0.5), data_std=((0.5, 0.5, 0.5)))
