__all__ = ['getLinkLayerTrainableVariable','getInnerLayerTrainableVariables','getAllLayer','getInnerLayer','sparsityCallback','calcul_sparsity']
...
def getInnerLayerTrainableVariables(layer):
    listinnerlayer=[]
    test_layer=[i for i in layer._layers if (isinstance(i,tf.keras.layers.Layer) or isinstance(i,tf.keras.Model))]
    if test_layer==[]:
        return [[layer,[i.name for i in layer.trainable_variables]]]
    else:
        for l in layer._layers:
            a=getInnerLayerTrainableVariables(l)
...
def calcul_sparsity(model):
    ...
    for layer in listlayer:
        if hasattr(layer,"kernel_mask"):
            stats.append([tf.math.count_nonzero(layer.kernel_mask).numpy(),tf.size(layer.kernel_mask).numpy()])
        if hasattr(layer,"bias_mask"):
            if layer.bias_mask is not None:
                stats.append([tf.math.count_nonzero(layer.bias_mask).numpy(),tf.size(layer.bias_mask).numpy()])
