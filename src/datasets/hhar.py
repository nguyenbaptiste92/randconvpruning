...
label_dict ={"null":0,"sit":2,"stand":3,"walk":4,"bike":6,"stairsup":7,"stairsdown":8}
...
def download_HHAR(data_dir,device=False):
    ...
    for filepath in glob.glob(path+os.sep+"*_accelerometer.csv"):
        ...
        for key,df in dico.items():
            ...
            dico_liste_sample[key] = [gb.get_group(x)[column] for x in gb.groups]
            
        #Filter Nan Echantillon,Transition Echantillon  et Echantillon with few value
        for key in dico_liste_sample.keys():
            #print(key)
            dico_liste_sample[key] = [elem for elem in dico_liste_sample[key] if not elem.isnull().values.any()]
            dico_liste_sample[key] = [elem.drop_duplicates(subset ='Creation_Time',keep = 'first', inplace = False) for elem in dico_liste_sample[key]]
            dico_liste_sample[key] = [elem for elem in dico_liste_sample[key] if not len(elem["gt"].unique())>1]
            len_liste=len(dico_liste_sample[key])
            if len_liste>0:
                max_lenght = max([len(elem)for elem in dico_liste_sample[key]])
            dico_liste_sample[key] = [elem for elem in dico_liste_sample[key] if (len(elem)>0.8*max_lenght and len(elem)>=40)]
        ...
        def SampleTo50Hz(df):
            ...
            new_df["Creation_Time"] = np.linspace(0, 3000000000, 150)+df["Creation_Time"].iloc[0]
            new_df["gt"] = df["gt"].iloc[0]
            new_df.iloc[:,1] = interpolate.interp1d(df["Creation_Time"], df.iloc[:,1], kind = 'linear',fill_value="extrapolate")(new_df["Creation_Time"])
            new_df.iloc[:,2] = interpolate.interp1d(df["Creation_Time"], df.iloc[:,2], kind = 'linear',fill_value="extrapolate")(new_df["Creation_Time"])
            new_df.iloc[:,3] = interpolate.interp1d(df["Creation_Time"], df.iloc[:,3], kind = 'linear',fill_value="extrapolate")(new_df["Creation_Time"])
            return new_df
        
        for key in dico_liste_sample.keys():
            dico_liste_sample[key] = [SampleTo50Hz(elem) for elem in dico_liste_sample[key]]
        
        #Assemble into a a dataset
        for key in dico_liste_sample.keys():
            if len(dico_liste_sample[key])>0:
                input_array = np.stack([elem.iloc[:,[1,2,3]].to_numpy() for elem in dico_liste_sample[key]])
                label_array = np.array([label_dict[elem.iloc[0,4]] for elem in dico_liste_sample[key]])
                
                #Add to position
                if key[1] not in dataset.keys():
    ...
    for key,dico in dataset.items():
        ...
        print(key,final_dataset[key]['label'].shape,final_dataset[key]['input'].shape)     
...
def get_HHAR(path,train=True,inter=True,proportion=0.8,transform=[],source_localisation="s3"):
