...
def download_realworld(data_dir, positional=False):
    ...
    if len(glob.glob(os.path.join(path, "csv_data") + os.sep + "*")) == 15:
        ...
        for filepath in glob.glob(path + os.sep + "*"):
            ...
            for zippath in glob.glob(
                os.path.join(filepath, "data") + os.sep + "acc_*_csv.zip",
            ):
                action_name = os.path.basename(os.path.normpath(zippath)).split(
                    "_",
                )[1]
    ...
    for folder_path in glob.glob(os.path.join(path, "csv_data") + os.sep + "*"):
        for action_path in glob.glob(folder_path + os.sep + "*"):
            ...
            for file_path in glob.glob(action_path + os.sep + "*.csv"):
                body_name = (
                    os.path.basename(os.path.normpath(file_path))
                    .split("_")[-1]
                    .split(".")[0]
                )
                ...
                df_list = [
                    df.iloc[n : n + 150, :] for n in range(0, len(df), 150)
                ]
                assert reduce(add, [len(elem) for elem in df_list]) == len(
                    df,
                ...
                print("Filter NaN.")
                df_list = [
                    elem for elem in df_list if not elem.isnull().values.any()
                ]
                print(len(df_list))
                ...
                input_array = np.stack(
                    [elem.iloc[:, [2, 3, 4]].to_numpy() for elem in df_list],
                )
                label_array = np.array(
                    [label_dict[action_name] for elem in df_list],
                )
                print(input_array.shape)
                print(label_array.shape)

                # Add to position
                if body_name not in dico_location:
