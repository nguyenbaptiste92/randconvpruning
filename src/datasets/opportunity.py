...
column_list = ["MILLISEC","Accelerometer RKN", "Accelerometer HIP" ,"Accelerometer LUA", "Accelerometer RUA" , "Accelerometer LH", "Accelerometer BACK", "Accelerometer RWR","Accelerometer LWR", "Accelerometer RH","InertialMeasurementUnit BACK acc","InertialMeasurementUnit RUA acc","InertialMeasurementUnit RLA acc","InertialMeasurementUnit LUA acc","InertialMeasurementUnit LLA acc","InertialMeasurementUnit L-SHOE Nav_A","InertialMeasurementUnit L-SHOE Body_A","InertialMeasurementUnit R-SHOE Nav_A","InertialMeasurementUnit R-SHOE Body_A","Locomotion"]

localisation_dict = {"UA":['RUA_', 'LUA^', 'LUA_', 'RUA^', 'RUA', 'LUA'],"LA":['LLA', 'RLA'],"WR":['LWR', 'RWR'],"H":['RH','LH'],"BACK":['BACK'],"HIP":['HIP'],"KN":['RKN_', 'RKN^'],"SHOE":['L-SHOE', 'R-SHOE']}
...
def get_column_names(data_dir,detailed=False):
    path = os.path.join(data_dir, "OpportunityUCIDataset","dataset",column_files)
    with open(path) as f:
        lines=f.readlines()
    ...
    if detailed:
        ...
        lines = [[line[0]] if len(line)==1 else [line[0],line[1].split(",")[1]] for line in lines]
        lines = [[line[0].split()[2:]] if len(line)==1 else [line[0].split()[2:],line[1].split()[2:]] for line in lines]
        lines = [[" ".join(line[0])] if len(line)==1 else [" ".join(line[0])," ".join(line[1])] for line in lines]
        lines = [line[0] if len(line)==1 else line[0]+" ("+line[1]+")" for line in lines]
    else:
    ...
    list_double = [item for item, count in collections.Counter(lines).items() if count > 1]
    dict_double_index = {item : [i for i, x in enumerate(lines) if x == item] for item in list_double}
    assert reduce(and_, [len(elem)==3 for elem in dict_double_index.values()]),"For this error, there should be triplet (for exemple Column: 165 Accelerometer MILK accX, Column: 166 Accelerometer MILK accX,  Column: 166 Accelerometer MILK accX)."
    indexes = reduce(concat,[[elem[1],elem[2]] for elem in dict_double_index.values()])
    replacements = reduce(concat,[[elem.replace("accX","accY"),elem.replace("accX","accZ")] for elem in dict_double_index.keys()])
...
def download_opportunity(data_dir,positional=False):
    ...
    if positional:
        pos_path = os.path.join(data_dir, "OpportunityUCIDataset", "dataset", "Positionale")
        ...
        glob_path = os.path.join(data_dir, "OpportunityUCIDataset", "dataset", "Globale")
    ...
    good_columns = [column for column in columns if reduce(or_, [column.startswith(elem) for elem in column_list])]
    
    dataset = {}
    for filepath in glob.glob(path + os.sep + "*.dat"):
        print("Read ",filepath)
        df= pd.read_csv(filepath,header = None, delim_whitespace=True, error_bad_lines=False, names=columns)
        df = df[good_columns]
        ...
        unique = [True if len(elem["Locomotion"].unique())==1 else False for elem in df_list]
        filtered_df_list = [elem for elem,condition in zip(df_list,unique) if condition==True]
        ...
        for loc,captors in localisation_dict.items():
            ...
            for captor in captors:
                captor_columns = ["MILLISEC"]+[elem for elem in good_columns if captor in elem]+["Locomotion"]
                captor_list_df = [elem[captor_columns] for elem in filtered_df_list]
                dico_location[loc] += [elem for elem in captor_list_df if not elem.isnull().values.any()]
        ...
        def SampleTo50Hz(df):
            ...
            new_df["MILLISEC"] = np.linspace(0, 2980, 150)+df["MILLISEC"].iloc[0]
            new_df["Locomotion"] = df["Locomotion"].iloc[0]
            new_df.iloc[:,1] = interpolate.interp1d(df["MILLISEC"], df.iloc[:,1], kind = 'linear',fill_value="extrapolate")(new_df["MILLISEC"])
            new_df.iloc[:,2] = interpolate.interp1d(df["MILLISEC"], df.iloc[:,2], kind = 'linear',fill_value="extrapolate")(new_df["MILLISEC"])
            new_df.iloc[:,3] = interpolate.interp1d(df["MILLISEC"], df.iloc[:,3], kind = 'linear',fill_value="extrapolate")(new_df["MILLISEC"])
            return new_df
...
def get_opportunity(path,train=True,inter=True,proportion=0.8,transform=[],source_localisation="H"):
