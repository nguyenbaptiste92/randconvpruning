...
IMU_column = ["Temperature","Accelerometer_1 X", "Accelerometer_1 Y", "Accelerometer_1 Z","Accelerometer_2 X","Accelerometer_2 Y","Accelerometer_2 Z","Gyroscope X","Gyroscope Y","Gyroscope Z","Magnetometer X","Magnetometer Y","Magnetometer Z","Orientation 1","Orientation 2","Orientation 3","Orientation 4"]
columns = ["Timestamp","Activity_ID","Heart Rate"]+["IMU hand "+string for string in IMU_column]+["IMU chest "+string for string in IMU_column]+["IMU ankle "+string for string in IMU_column]

number_to_label_dict = {0:"null",1:"lie",2:"sit",3:"stand",4:"walk",5:"run",6:"bike",7:"nordic_walking",12:"stairsup",13:"stairsdown",16:"vacuum",17:"iron",24:"rope_jumping"}
label_dict ={"null":0,"lie":1,"sit":2,"stand":3,"walk":4,"run":5,"bike":6,"stairsup":7,"stairsdown":8,"nordic_walking":10,"vacuum":11,"iron":12,"rope_jumping":13}
...
def download_PAMAP2(data_dir,positional=False):
    ...
    if positional:
        pos_path = os.path.join(data_dir, "PAMAP2_Dataset", "Protocol", "Positionale")
        ...
        glob_path = os.path.join(data_dir, "PAMAP2_Dataset", "Protocol", "Globale")
    ...
    for filepath in glob.glob(path + os.sep + "*.dat"):    
        ...
        unique = [True if len(elem["Activity_ID"].unique())==1 else False for elem in df_list]
        ...
        filtered_df_list = [elem for elem in filtered_df_list if elem["Activity_ID"].iloc[0] in [1,2,3,4,5,6,7,12,13,16,17,24]]
        print(len(filtered_df_list))
        
        #Separate by captor location then filter echantillon with NaN
        print("Separate by accelerometer location")
        locations = list(set([elem.split()[1] for elem in good_columns if "Accelerometer" in elem]))
        dico_location={}
        for loc in locations:
            loc_columns = ["Timestamp","Activity_ID"]+[elem for elem in good_columns if loc in elem]
            dico_location[loc] = [elem[loc_columns] for elem in filtered_df_list]
            dico_location[loc] = [elem for elem in dico_location[loc] if not elem.isnull().values.any()]
        ...
        def SampleTo50Hz(df):
            ...
            new_df["Timestamp"] = np.linspace(0, 2980, 150)+df["Timestamp"].iloc[0]
            new_df["Activity_ID"] = df["Activity_ID"].iloc[0]
            new_df.iloc[:,2] = interpolate.interp1d(df["Timestamp"], df.iloc[:,2], kind = 'linear',fill_value="extrapolate")(new_df["Timestamp"])
            new_df.iloc[:,3] = interpolate.interp1d(df["Timestamp"], df.iloc[:,3], kind = 'linear',fill_value="extrapolate")(new_df["Timestamp"])
            new_df.iloc[:,4] = interpolate.interp1d(df["Timestamp"], df.iloc[:,4], kind = 'linear',fill_value="extrapolate")(new_df["Timestamp"])
            return new_df
        ...
        for key,liste in dico_location.items():
            ...
            if len(liste)>0:
                ...
                input_liste = [elem.iloc[:,[2,3,4]].to_numpy() for elem in liste]
...
def get_PAMAP2(path,train=True,inter=True,proportion=0.8,transform=[],source_localisation="ankle"):
